...
payment_Insert.sendFiles=  function()
{
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200 ) {
            ...
            $('#loadingBar').hide();
            $('#fileStatus').hide();
            payment_Insert.Submit(currentButton);
        } else {
    ...
    xhr.open('POST', "Home/SendFiles?PageName=payment_Insert");
    // xhr.setRequestHeader("Content-type", "multipart/form-data");
    xhr.send(data);
}


payment_Insert.Submit= function(obj)
{
    currentButton=obj;
    $(obj).attr('disabled',true);
    if(payment_Insert.Validate()==false)
    {
...
payment_Insert.Validate= function()
{
...
payment_Insert.Serach=function(obj)
{
    $(obj).attr('disabled',true);
    if(payment_Insert.Validate()==false)
    {
    ...
    window.CurrentSerachMethod=payment_Insert.Serach;
    var Entity=new Object();
    Entity.PageName='payment_Insert';
    Entity.Parameters=new Array();
                Entity.Parameters.push( toInput('payment_amount',$('#txtpayment_Insertpayment_amount').val()));
